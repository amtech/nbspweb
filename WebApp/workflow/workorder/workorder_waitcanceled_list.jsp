...
In.add('layoutjs',{path:'${ctx}/js/jQuery/layout/jquery.layout-latest.min.js',type:'js',charset:'utf-8',rely:['layout']});
  ...
  var jqgrid=jQuery("#workordergrid").jqGrid({    
		url: "${ctx}/workflow/workOrderCancelAction!waitCanceledListdata.action",   
		datatype: "json",mtype: 'POST',rownumbers: true,
		colNames:['工单标题','工单号','现场处理人','处理时限','工单类型', '派单人','派单时间','状态','受理人','操作','PID'],
		colModel:[
		          ...
		          {name:'TARGET_PRINCIPAL_NAME',id:'TARGET_PRINCIPAL_NAME',sortable:false,hidden:true},
		          {name:'ID',id:'ID',sortable:false,formatter:WaitCanceledActionFmatter,width:80},
...
<body>
	<form action="${ctx }/workflow/workOrderCancelAction!waitCanceledList.action" id="workorderForm" name="form" method="post">
		<div id="header">
			<div class="title_bg">
				<div id="title" class="title">当前位置-待取消任务</div>
			</div>
			<div id="accordion">
				<h3>
				...
				<div class="tabcontent">
					<table cellspacing="0" cellpadding="0" border="0" align="center">
						...
						<tr>
							<th>工单类型：</th>
							<td>
								<baseinfo:dicselector name="taskType" columntype="TASK_CODE" type="select" style="width:220px;" cssClass="inputtext" isQuery="query"></baseinfo:dicselector>
							</td>
							<th>流程状态：</th>
							<td>
								<baseinfo:customselector name="taskState" data="${taskStates}" isReversal="true" style="width:220px;" isQuery="query" cssClass="inputtext"></baseinfo:customselector>
							</td>
						</tr>
						<tr>
							<td colspan="4" align="center">
								<input type="button" onclick="query();" class="button" value="查询" />
								<baseinfo:expexcel cls="exprotButton"  businessId="workorder_waitcanceled_list" name="导出excel"></baseinfo:expexcel>
							</td>
						</tr>
					</table>
				</div>
	...
	function query() {
		jQuery("#workordergrid").jqGrid().setGridParam({
            postData: {
            	...
            	taskState:jQuery('select[name="taskState"]').find("option:selected").val()
            	} 
            }).trigger("reloadGrid");
	}
</script>
