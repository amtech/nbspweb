...
In.add('fault-list', {path : '${ctx}/workflow/fault/js/fault_dispatch_waithandled_list.js',type : 'js',charset : 'utf-8',rely:['fault-common']});
...
In.add('layoutjs',{path:'${ctx}/js/jQuery/layout/jquery.layout-latest.min.js',type:'js',charset:'utf-8',rely:['layout']});
	...
	accordionObj=jQuery("#accordion").accordion({
		...
		var jqgrid=jQuery("#faultgrid").jqGrid({    
			url: "${ctx}/workflow/faultDispatchAction!waitHandledListdata.action?",   
			datatype: "json",mtype: 'GET',rownumbers: true,
			colNames:['故障单标题','工单号','EOMS单号', '故障发生时间','故障发生地点','故障站点','是否紧急','发现方式','处理人','处理时限','状态','操作','专业类型','处理URL','处理ID'],
			colModel:[
			          ...
			          {name:'PROINST_STATE',id:'PROINST_STATE',sortable:false,width:80},
			          {name:'ID',id:'ID',sortable:false,formatter:waitHandledFmatter,width:60},
			          {name:'BUSINESS_TYPE',id:'BUSINESS_TYPE',sortable:false,hidden:true},
			          {name:'PROCESS_URL',id:'PROCESS_URL',sortable:false,hidden:true},
			          {name:'DISPATCH_ID',id:'DISPATCH_ID',sortable:false,hidden:true}
			          ],      
		...
		}).navGrid('#faultpager',{edit:false ,add:false ,del:false,search:false,sortable:false  });
						...
						<table cellspacing="0" cellpadding="0" border="0" align="center">
							...
							<tr>
								<td align="center" colspan="4">
									...
									<baseinfo:expexcel cls="exprotButton" businessId="faultdispatchwaithandledlist" name="导出excel"></baseinfo:expexcel>
								</td>
							</tr>
						</table>
					</div>
