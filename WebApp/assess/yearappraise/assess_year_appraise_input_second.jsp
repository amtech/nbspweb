...
<body>
	<form action="${ctx }/assess/assessYearAppraiseAction!save.action"
		id="inputForm" name="inputForm" method="post">
		<div class="title_bg">
			<div id="title" class="title">当前位置-年度考核-考核评分</div>
		</div>
		<div class="tabout">
			...
			<table border="0" align="center" cellpadding="3" cellspacing="0">
				...
				<tr>
					...
					<th>评估考核时间</th>
					<td>
						<fmt:formatDate value="${assessExaminationResult.appraiseMonth }" pattern="yyyy年" />
					</td>
				</tr>
				<tr><td colspan="4">
					<table>
						...
						<tbody>
							<c:forEach var="item" items="${templatecontent}" varStatus="status">
								<tr>
									...
									<td style="text-align: center;">
										<input id="assessExaminationResult_itemScore${item.CONTENTID}" name="assessExaminationResult.itemScore" value="${item.SCORE }" type="text" style="width:80px" class="required" />
									</td>
									<td style="text-align: center;">
										<c:set var="desc" value="无"></c:set>
										<c:if test="${not empty item.RATING_DESC }"><c:set var="desc" value="${item.RATING_DESC }"></c:set></c:if>
										<input id="assessExaminationResult_ratingDesc${item.CONTENTID}" name="assessExaminationResult.ratingDesc" style="width:80px" value="${desc }" type="text" class="required" />
									</td>
								</tr>
							</c:forEach>
						</tbody>
