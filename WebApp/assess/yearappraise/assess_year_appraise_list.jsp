...
In.add('wdatejs',{path:'${ctx}/js/WdatePicker/WdatePicker.js',type:'js',charset:'utf-8'});
	In.ready('jgcn', 'common', 'wdatejs', function() {
		...
		var jqgrid=jQuery("#assessgrid").jqGrid({    
			url: "${ctx}/assess/assessYearAppraiseListAction!listData.action?",   
			datatype: "json",    
			mtype: 'GET',
			rownumbers: true,
			colNames:['考核表名称','考核年份','专业类型', '代维公司','考核成绩','考核人','考核时间','操作'],
			colModel:[
			          {name:'TABLE_NAME',id:'TABLE_NAME',sortable:false},
			          {name:'YEAR',id:'YEAR',sortable:false},
	...
	function view(id){
		window.location.href = "${ctx}/assess/assessYearAppraiseAction!view.action?id="+id+"&tableType=01";
	}
