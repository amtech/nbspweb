...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<form id="searchForm" name="searchForm">
			...
			<div align="left">
				...
				<baseinfo:expexcel  businessId="onlinelogresult" name="导出excel"></baseinfo:expexcel>
			</div>
	...
	function del(id) {
		if (confirm("删除将不能恢复，请确认是否要删除该用户在线信息查询，按‘确定’删除？")) {
			window.location.href = '${ctx}/system/onlinelog!del.action?ids=' + id;
		}
	}
	function batchDel(){
		var idValue=""; 
	    ...
	    if (confirm("删除将不能恢复，请确认是否要删除选中的用户在线信息查询，按‘确定’删除？")) {
			window.location.href = '${ctx}/system/onlinelog!del.action?ids=' + idValue;
		}
	}
	In.ready('jgcn', 'common', 'wdatejs', 'fault-common', function() {
		...
		var jqgrid = jQuery("#noticegrid").jqGrid({
			url : "${ctx}/system/onlinelog!listData.action",
			datatype : "json",
			mtype : 'POST',
			rownumbers : true,
			colNames : [ '用户名称', '用户IP', '登录时间', '登出时间', '在线时长','操作' ],
			colModel : [ {
				name : 'PERSONID',id : 'PERSONID',sortable : false
			}, {
				name : 'IP',id : 'IP',sortable : false
			}, {
				name : 'LOGIN_TIME',id : 'LOGIN_TIME',sortable : false
			}, {
				name : 'LOGOUT_TIME',id : 'LOGOUT_TIME',sortable : false
			}, {
				name : 'ONLINE_TIME',id : 'ONLINE_TIME',sortable : false
			}, {
