...
<title>故障平均时长统计</title>
<script>
	jQuery(function(){
	//使用层布局
  var jqgrid=jQuery("#troublegrid").jqGrid({    
		url: "${ctx}/analysis/troubleAnalysisAction!timelengthlist.action",   
		datatype: "json",    
		mtype: 'GET',
		rownumbers: true,
		colNames:['ID','名称','平均历时（小时）','总历时（小时）','严重故障平均历时（小时）','严重故障总历时（小时）', '重大故障平均历时（小时）','重大故障总历时（小时）','一般故障平均历时（小时）','一般故障总历时（小时）','工单总数'],
		colModel:[
		          {name:'ID',id:'ID',sortable:false,hidden:true},
		          {name:'NAME',id:'NAME',sortable:false},
		          {name:'ALL_AVG_LEN',id:'ALL_AVG_LEN',sortable:false},
		          {name:'ALL_LEN',id:'ALL_LEN',sortable:false},
		          {name:'LEVEL_AVG_LEN0',id:'LEVEL_AVG_LEN0',sortable:false},
		          {name:'LEVEL_LEN0',id:'LEVEL_LEN0',sortable:false},
		          {name:'LEVEL_AVG_LEN1',id:'LEVEL_AVG_LEN1',sortable:false},
		          {name:'LEVEL_LEN1',id:'LEVEL_LEN1',sortable:false},
		          {name:'LEVEL_AVG_LEN2',id:'LEVEL_AVG_LEN2',sortable:false},
		          {name:'LEVEL_LEN2',id:'LEVEL_LEN2',sortable:false},
        ...
        subGridRowExpanded: function(subgrid_id, row_id) {
        	            ...
        	            jQuery("#" + subgrid_table_id).jqGrid({  
        	                url: "${ctx}/analysis/troubleAnalysisAction!timelengthsublist.action?orgid="+row_id,  //   
        	                datatype: "json",  
        	            	colNames:['ID','名称','平均历时（小时）','总历时（小时）','严重故障平均历时（小时）','严重故障总历时（小时）', '重大故障平均历时（小时）','重大故障总历时（小时）','一般故障平均历时（小时）','一般故障总历时（小时）','工单总数'],
        	        		colModel:[
        	        		          {name:'ID',id:'ID',sortable:false,hidden:true},
...
<body>
	<form id="troubleForm" name="troubleForm">
		<div id="header">
			<div class="title_bg">
				<div id="title" class="title">当前位置-故障平均时长统计</div>
			</div>
