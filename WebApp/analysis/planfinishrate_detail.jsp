...
<script>
  ...
  var jqgrid=jQuery("#inplangrid").jqGrid({    
		url: "${ctx}/analysis/planDetailAction!planinfinishlist.action",   
		datatype: "json",    
		mtype: 'GET',
		rownumbers: true,
		colNames:['ID','站点编号','站点名称','专业类型', '站点维护单位','站点维护组','巡检日期','巡检时长(秒)'],
		colModel:[
		          ...
		          {name:'PATROLDATE',id:'PATROLDATE',sortable:false},
		          {name:'TAKETIME',id:'TAKETIME',sortable:false}
		          ],      
		autowidth:true,
		shrinkToFit:true,
		viewrecords: true,
		...
		jsonReader: {
		  ...
		  }).navGrid('#inplangrid',{edit:false ,add:false ,del:false,search:false,sortable:false  });
	var initialized = false; //是否已初始化
	tab.bind('tabsshow', function(event, ui) {
		//选择二个tab时，生成未巡检rfid
		if (ui.index == 1 && !initialized){ 
	jQuery("#overplangrid").jqGrid({    
		url: "${ctx}/analysis/planDetailAction!planoverfinishlist.action",
		datatype: "json",    
		mtype: 'POST',
		colNames:['ID','站点编号','站点名称','专业类型', '站点维护单位','站点维护组'],
		colModel:[
		          {name:'ID',id:'ID',sortable:false,hidden:true},
		          {name:'STATIONCODE',id:'STATIONCODE',sortable:false},
		          {name:'RSNAME',id:'RSNAME',sortable:false},
...
<body>
	<form id="planForm" name="planForm">
		<div id="header">
			<div class="title_bg">
				<div class="title">当前位置-计划资源完成率明细</div>
			</div>
		...
		<div id="content" align="center" style="padding-top: 2px">
			<div id="tab_process">
				<ul>
					<li><a href="#inplan">已巡检站点</a></li>
					<li><a href="#overplan">未覆盖站点</a></li>
