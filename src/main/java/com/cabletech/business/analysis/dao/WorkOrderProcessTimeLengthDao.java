...
@Repository
public class WorkOrderProcessTimeLengthDao extends WorkOrderAnalyseBaseDao {
	/**
	 * 进行工单时长统计（按组织分组）
	 * 
	 * @param parameters
	 ...
	 * @return
	 */
	public List getWorkOrderProcessTimeLengthListByOrg(
		...
		sqlBuffer.append(getWorkOrderProcessTimeLengthSqlByOrg(parameters));
		sqlBuffer.append(" ) result_ on vo.id=result_.orgid ");
		return super.getSQLALL(sqlBuffer.toString());
	}

	/**
	 * 获取工单处理及时率统计的sql（按组织分组）
	 * 
	 * @param parameters
	 *            Map<String, String>
	 * @return
	 */
	private StringBuffer getWorkOrderProcessTimeLengthSqlByOrg(
			Map<String, String> parameters) {
		StringBuffer sqlBuffer = new StringBuffer("");
		sqlBuffer.append(" select all_t.orgid, ");
		sqlBuffer.append(" count(all_t.pid) as all_num, ");
		sqlBuffer.append(" count(intime_t.pid) as intime_num, ");
		sqlBuffer.append(" count(overtime_t.pid) as overtime_num, ");
		sqlBuffer
				.append(" to_char(sum(all_t.len),'FM99999990.099') as all_len, ");
		sqlBuffer
				.append(" to_char(sum(intime_t.len),'FM99999990.099') as intime_len, ");
		sqlBuffer
				.append(" to_char(sum(overtime_t.len),'FM99999990.099') as overtime_len, ");
		sqlBuffer
				.append(" to_char(decode(count(all_t.pid),0,0,sum(all_t.len)/count(all_t.pid)),'FM99999990.099') as all_avg_len, ");
		sqlBuffer
				.append(" to_char(decode(count(intime_t.pid),0,0,sum(intime_t.len)/count(intime_t.pid)),'FM99999990.099') as intime_avg_len, ");
		sqlBuffer
		...
		sqlBuffer.append(getInTimeWorkOrderSql(parameters));
		sqlBuffer.append(" ) intime_t on intime_t.pid=all_t.pid ");
		sqlBuffer.append(" left join ( ");
	 ...
	 * @return
	 */
	private String getInTimeWorkOrderSql(Map<String, String> parameters) {
		// TODO Auto-generated method stub
		StringBuffer sqlBuffer = new StringBuffer("");
		sqlBuffer
