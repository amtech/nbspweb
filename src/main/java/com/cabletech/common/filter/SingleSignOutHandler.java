...
public class SingleSignOutHandler {
	...
	public void init() {
		...
		CommonUtils.assertNotNull(this.logoutParameterName,
				"logoutParameterName cannot be null.");
		CommonUtils.assertNotNull(this.sessionMappingStorage,
				"sessionMappingStorage cannote be null.");
	}
	...
	public boolean isLogoutRequest(final HttpServletRequest request) {
		return ("GET".equals(request.getMethod()) || "POST".equals(request
				.getMethod()))
		...
		if (CommonUtils.isNotBlank(token)) {
			...
			if (session != null) {
				String sessionID = session.getId();
				 WebApplicationContext web = WebApplicationContextUtils.getWebApplicationContext(session.getServletContext());
	        	 UserOnlineLogService uolservice=(UserOnlineLogService)web.getBean("userOnlineLogServiceImpl");
	        	 UserInfo user =(UserInfo)session.getAttribute("LOGIN_USER");
		         uolservice.UpdateLogoutTime(user.getPersonId());		
				if (log.isDebugEnabled()) {
