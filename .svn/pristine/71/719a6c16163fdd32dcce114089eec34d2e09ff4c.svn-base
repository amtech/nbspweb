...
<script>
  ...
  var jqgrid=jQuery("#introublegrid").jqGrid({    
		url: "${ctx}/analysis/troubleDetailAction!responsetimelyinlist.action",   
		datatype: "json",    
		mtype: 'GET',
		rownumbers: true,
		colNames:['ID','故障单标题','EMOS单号','故障发生时间', '故障发生地点','故障级别','发现方式','响应时长（小时）'],
		colModel:[
	...
	jQuery("#overtroublegrid").jqGrid({    
		url: "${ctx}/analysis/troubleDetailAction!responsetimelyoverlist.action", 
		datatype: "json",    
		mtype: 'POST',
		colNames:['ID','故障单标题','EMOS单号','故障发生时间', '故障发生地点','故障级别','发现方式','响应时长'],
		colModel:[
		          {name:'ID',id:'ID',sortable:false,hidden:true},
		          {name:'TITLE',id:'TITLE',sortable:false},
		          {name:'EOMS_ID',id:'EOMS_ID',sortable:false},
...
<body>
	<form id="troubleForm" name="troubleForm">
		<div id="header">
			<div class="title_bg">
				<div class="title">当前位置-故障处理响应明细</div>
			</div>
		...
		<div id="content" align="center" style="padding-top: 2px">
			<div id="tab_process">
				<ul>
					<li><a href="#introuble">响应及时工单</a></li>
					<li><a href="#overtrouble">响应超时工单</a></li>
