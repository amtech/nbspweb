...
@Repository
public class WorkOrderProcessInTimeRateDao extends WorkOrderAnalyseBaseDao {
	/**
	 * 进行工单处理及时率统计（按组织分组）
	 * 
	 * @param parameters
	 ...
	 * @return
	 */
	public List getWorkOrderProcessInTimeRateListByOrg(
	...
	public Page getInTimeWorkOrderList(Page page, Map<String, String> parameters) {
		String sql = getInTimeWorkOrderSql(parameters);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Page getOverTimeWorkOrderList(Page page,
			Map<String, String> parameters) {
		String sql = getOverTimeWorkOrderSql(parameters);
		return super.getSQLPageAll(page, sql);
	}
	...
	private StringBuffer getWorkOrderProcessInTimeRateSqlByOrg(
		...
		sqlBuffer.append(" to_char(decode(count(all_t.pid),0,0, ");
		sqlBuffer.append(" 100*count(intime_t.pid)/count(all_t.pid) ");
