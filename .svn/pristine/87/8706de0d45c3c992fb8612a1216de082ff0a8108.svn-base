...
<body>
	<form id="patrolinfoForm" name="patrolinfoForm">
		<div class="title_bg">
			<div id="title" class="title">当前位置-计划执行结果</div>
		</div>
				...
				<tr>
					<td colspan="4" align="center"><input type="button"
						...
						type="button" onclick="reset();" class="button" value="清除" /> <baseinfo:expexcel
							cls="exprotButton" businessId="patrolinfoexecuteresultlist"
...
In.ready('jgcn', 'common', 'wdatejs', 'patrolinfo_common', function() {
	...
	var jqgrid=jQuery("#patrolinfogrid").jqGrid({    
		url: "${ctx}/wplan/patrolinfoExecuteAction!queryshowresult.action?type="+jQuery('select[name="businesstype"]').val(),   
		datatype: "json",    
		mtype: 'GET',
		rownumbers: true,
		colNames:['计划ID','计划名称','专业类型','所属区域', '所属代维','所属巡检组','计划开始时间','计划结束时间','计划巡检数','巡检异常数','已巡检数','未巡检数','巡检完成率'],
		colModel:[
		          ...
		          {name:'START_TIME',id:'START_TIME',sortable:false,width:90,formatter:'date',formatoptions: {newformat:'Y-m-d'} },
		          {name:'END_TIME',id:'END_TIME',sortable:false,width:90,formatter:'date',formatoptions: {newformat:'Y-m-d'},summaryType:'sum',summaryTpl:'<b>合计：</b>' },
		          {name:'PATROLCOUNT',id:'PATROLCOUNT',sortable:false,width:80,formatter:'integer',sorttype:'int', summaryType:'sum',summaryTpl:'<b>{0}</b>'},
		          {name:'EXCEPTIONCOUNT',id:'EXCEPTIONCOUNT',sortable:false,width:80,formatter:'integer',sorttype:'int', summaryType:'sum',summaryTpl:'<b>{0}</b>'},
		          {name:'ENDPATROLCOUNT',id:'ENDPATROLCOUNT',sortable:false,width:80,formatter:OverPatrolFmatter,sorttype:'int', summaryType:'sum',summaryTpl:'<b>{0}</b>'},
		          {name:'NOPATROLCOUNT',id:'NOPATROLCOUNT',sortable:false,width:80,formatter:LosePatrolFmatter,sorttype:'int', summaryType:'sum',summaryTpl:'<b>{0}</b>'},
