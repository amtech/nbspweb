...
In.add('wdatejs',{path:'${ctx}/js/WdatePicker/WdatePicker.js',type:'js',charset:'utf-8'});
	In.ready('jgcn', 'common', 'wdatejs', function() {
		...
		var jqgrid = jQuery("#analysegrid")
				.jqGrid(
						{
							url : "${ctx}/analysis/problemStationAnalysisAction!listData.action?",
							datatype : "local",
							mtype : 'GET',
							rownumbers : true,
							colNames : [ '站点名称', '专业类型', '异常项总数', '地址','所属代维','代维负责人','联系电话','操作' ],
							colModel : [ 
							...
							{name : 'ORG_LINKMAN',id : 'ORG_LINKMAN',sortable : false}, 
							{name : 'ORG_TEL',id : 'ORG_TEL',sortable : false},
					...
					<table cellspacing="0" cellpadding="0" border="0" align="center">
						...
						<tr>
							<td align="center" colspan="4">
								...
								<baseinfo:expexcel cls="exprotButton" businessId="problemStationList" name="导出excel"></baseinfo:expexcel>
							</td>
						</tr>
					</table>
				</div>
...
function viewPatrolProblem(stationId,businessType){
	var orgId="&org="+jQuery("#orgId").val();
	var regionId="&regionId="+jQuery("#regionId").val();
	var businessTypeQ="&businessType="+businessType;
	var patrolId="&patrolId="+jQuery("#patrolId").val();
	var startTime="&startTime="+jQuery("#startTime").val();
	var endTime="&endTime="+jQuery("#endTime").val();
	var queryString=orgId+regionId+businessTypeQ+patrolId+startTime+endTime;
	window.location.href = "${ctx}/analysis/problemStationAnalysisAction!problemList.action?id="+stationId+queryString;
}
function viewFormatter(cellValue,options,rowObject){
	var view="<a style='color: blue;text-decoration: underline;' href=javascript:viewPatrolProblem('"+rowObject.STATION_ID+"','"+rowObject.BUSINESSTYPE+"')>历史信息</a>";;
	return view;
}
	</script>
</html>
