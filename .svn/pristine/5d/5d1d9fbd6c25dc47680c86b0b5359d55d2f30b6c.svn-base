...
In.ready('jgcn', 'common', 'wdatejs','ztreejs','ztreeui', function() {
		...
		var jqgrid = jQuery("#itemgrid")
				.jqGrid(
						{
							url : "${ctx}/wplan/patrolinfoResultAction!orgDetailsData.action?regionId=${parameters['regionId']}&startTime=${parameters['startTime']}&orgId=${parameters['orgId']}&patrolGroupId=${parameters['patrolGroupId']}&endTime=${parameters['endTime']}&businessType=${parameters['businessType']}",
							datatype : "json",
							mtype : 'GET',
							rownumbers : true,
							colNames : [ '代维公司', '计划站点数', '问题站点数', '拆迁站点数', '纠纷站点数', '其它不能巡检站点数' ],
							colModel : [{
								name : 'ORGNAME',
								id : 'ORGNAME',
								sortable : false,
								formatter:patrolgroupDetailsFmt
							}, {
								name : 'PLANNUB',
								id : 'PLANNUB',
								sortable : false,
...
function patrolgroupDetailsFmt(cellvalue, options, rowObjec){
		var view = "<a style='color: blue;text-decoration: underline;' href=javascript:patrolgroupDetailsShowDetailed('"
		+ rowObjec.ID+"','"+rowObjec.BUSINESSTYPE+"','"+rowObjec.STARTTIME+"','"+rowObjec.ENDTIME+ "')>"+cellvalue+"</a>";
		return view;
}
function patrolgroupDetailsShowDetailed(id,businessType,startTime,endTime){
		...
		window.location.href = "${ctx}/wplan/patrolinfoResultAction!patrolgroupDetails.action?problemType=01&orgId="+id+"&businessType="+businessType+"&startTime="+startTime+"&endTime="+endTime;
}
...
function cqShowDetailed(id,businessType,startTime,endTime){
		...
		window.location.href = "${ctx}/wplan/patrolinfoResultAction!noPatrolDetails.action?problemType=01&orgId="+id+"&businessType="+businessType+"&startTime="+startTime+"&endTime="+endTime;
}
...
function jfShowDetailed(id,businessType,startTime,endTime){
		...
		window.location.href = "${ctx}/wplan/patrolinfoResultAction!noPatrolDetails.action?problemType=02&orgId="+id+"&businessType="+businessType+"&startTime="+startTime+"&endTime="+endTime;
}
...
function qtShowDetailed(id,businessType,startTime,endTime){
		...
		window.location.href = "${ctx}/wplan/patrolinfoResultAction!noPatrolDetails.action?problemType=03&orgId="+id+"&businessType="+businessType+"&startTime="+startTime+"&endTime="+endTime;
}
...
function showProblemResFmt(id,businessType,startTime,endTime){
		...
		window.location.href = "${ctx}/wplan/patrolinfoResultAction!problemStationList.action?orgId="+id+"&businessType="+businessType+"&startTime="+startTime+"&endTime="+endTime;
}
...
function showPlanFmt(id,businessType,startTime,endTime){
		...
		window.location.href = "${ctx}/wplan/patrolinfoResultAction!planInfoList.action?orgId="+id+"&businessType="+businessType+"&startTime="+startTime+"&endTime="+endTime;
}
</script>
</head>
<body>
	<form id="optForm" method="post"
		...
		<div class="title_bg">
			<div id="title" class="title">当前位置-组织巡检结果明细</div>
		</div>
					...
					<table cellspacing="0" cellpadding="0" border="0" align="center">
						<tr>
							<td align="center" colspan="4">
								...
								<baseinfo:expexcel cls="exprotButton" businessId="orgpatrolresultstatistics" name="导出excel"></baseinfo:expexcel>
							</td>
						</tr>
					</table>
		</div>
