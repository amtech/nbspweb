...
<script type="text/javascript">
	jQuery(function() {
		...
		jQuery("#tongji1")
				.click(
						function() {
							...
							var url = "${ctx}/monthcost/monthstatistical!staticByType.action?type=Times&lab=1&month="
									+ month + "&year=" + year;
							window.location = url;
		...
		jQuery("#tongji2")
				.click(
						function() {
							...
							var url = "${ctx}/monthcost/monthstatistical!staticByType.action?type=Times&lab=2&month="
									+ month + "&year=" + year;
							window.location = url;
		...
		jQuery("#tongji3")
				.click(
						function() {
							...
							var url = "${ctx}/monthcost/monthstatistical!staticByType.action?type=Times&lab=3&month="
									+ month + "&year=" + year;
							window.location = url;
		...
		jQuery("#tongji4")
				.click(
						function() {
							var year = jQuery("#qyear").val();
							var month = jQuery("#qmonth").val();
							if (year == "") {
								alert("年份不可以不选择");
								return false;
							}
							...
							var url = "${ctx}/monthcost/monthstatistical!staticByType.action?type=Times&lab=4&month="
									+ month + "&year=" + year;
							window.location = url;
							jQuery("#qyear").val('${year}');
							jQuery("#qmonth").val('${month}');
						});
		for ( var i = 0; i <= 4; i++) {
			table_rowspan("#table4Search4", i);
		}
	});	
	function exportLs(type,n){ 
		...
		if(n=='1'){
			  ...
			  year = jQuery("#qyear").val();
			  month = jQuery("#qmonth").val();
		}
...
<body>
	<div class="title_bg">
		<div class="title">当前位置-计次服务费用统计</div>
	</div>
	<div class="tabbable" style="background: #ffffff; padding: 10px;">
		<ul class="nav nav-tabs">
			...
			<li <c:if test="${labq=='3'}">class='active'</c:if>><a
				href="#tab1-3" data-toggle="tab" id="lab3">按代维专业统计</a></li>
			<li <c:if test="${labq=='4'}">class='active'</c:if>><a
				href="#tab1-4" data-toggle="tab" id="lab4">按计次类型统计</a></li>
		</ul>
		<div class="tab-content"
			...
			<div class="tab-pane  <c:if test="${labq=='1'}"> active  </c:if>"
				...
				<p>
					年份:<select id="ryear" name="ryear">
						...
						type="button" value="导出"  onclick="exportLs('Times','1')"/>
				</p>
				<p>
				<table width="100%" height="98%" border="1" cellpadding="0"
					...
					<thead>
						<tr>
							...
							<td align="center"><strong>代维专业</strong></td>
							<td align="center"><strong>计次类型</strong></td>
							<td align="center"><strong>单价（元）</strong></td>
							<td align="center"><strong>计次数量</strong></td>
					...
					<tbody id="tbody1">
						<c:forEach var="map" items="${DataMap}" varStatus="status">
							<tr>
								...
								<td align="center">${map.SPECIALTY }</td>
								<td align="center">${map.TYPET }</td>
								<td align="center">${map.UNITPRICE }</td>
								<td align="center">${map.NUMBERS }</td>
			...
			<div class="tab-pane  <c:if test="${labq=='2'}"> active  </c:if>"
				...
				<p>
					年份:<select id="cyear" name="cyear"><option value="">--请选择--</option>
						...
						type="button" value="导出"  onclick="exportLs('Times','2')"/>
				</p>
				<p>
				<table width="100%" height="98%" border="1" cellpadding="0"
			...
			<div class="tab-pane  <c:if test="${labq=='3'}"> active  </c:if>"
				...
				<p>
					年份:<select id="syear" name="syear"><option value="">--请选择--</option>
						...
						type="button" value="导出"  onclick="exportLs('Times','3')"/>
				</p>
				<p>
				<table width="100%" height="98%" border="1" cellpadding="0"
			...
			<div class="tab-pane  <c:if test="${labq=='4'}"> active  </c:if>"
				id="tab1-4">
				<p>
					年份:<select id="qyear" name="qyear"><option value="">--请选择--</option>
						<option value="2012"
							<c:if test="${year==\'2012\'}">selected='selected'</c:if>>2012</option>
						<option value="2013"
							<c:if test="${year=='2013' }">selected='selected'</c:if>>2013</option>
						<option value="2014"
							<c:if test="${year =='2014'}">selected='selected'</c:if>>2014</option>
					</select> 月度:<select id="qmonth" name="qmonth"><option value="">--请选择--</option>
						<option value="1"
							<c:if test="${month =='1'}">selected='selected'</c:if>>1</option>
						<option value="2"
							<c:if test="${month =='2'}">selected='selected'</c:if>>2</option>
						...
						<option value="12"
							<c:if test="${month =='12'}">selected='selected'</c:if>>12</option>
					</select> <input type="button" value="查询" id="tongji4" /> <input
						type="button" value="导出"  onclick="exportLs('Times','4')"/>
				</p>
				<p>
				<table width="100%" height="98%" border="1" cellpadding="0"
					id="table4Search4" cellspacing="0" bordercolor="#000000">
					<thead>
						<tr>
							<td align="center"><strong>计次类型</strong></td>
							<td align="center"><strong>代维专业</strong></td>
