...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	jQuery(function() {
		...
		var jqgrid = jQuery("#grid").jqGrid(
				{
					url : "${ctx}/monthcost/monthcheckcost!listDate.action",
					datatype : "json",
					mtype : 'POST',
					rownumbers : true,
					colNames : [ '区域', '代维公司', '代维专业', '代维数量', '单价', '考核分数',
							'扣款', '应付款', '实付款', '月份', '操作' ],
					colModel : [ {
						name : 'REGIONNAME',
						id : 'REGIONNAME',
	...
	function toView(idValue) {
		var URL = "${ctx}/monthcost/monthcheckcost!view.action?id=" + idValue;
	...
	function toDelete(idValue) {
		if (confirm("确定删除该纪录？")) {
			var url = "${ctx}/monthcost/monthcheckcost!delete.action?id="
					+ idValue;
	...
	function toEdit(idValue) {
		var url = "${ctx}/monthcost/monthcheckcost!input.action?id=" + idValue;
		self.location = url;
					...
					<ul class="searchul">
						<li class="searchli">
							...
							<baseinfo:customselector name="entity.specialty"
								isReversal="true" data="${businessTypeMap }"
								cssClass="inputtext required" id="specialty"
								style="width: 180px" keyValue=""></baseinfo:customselector>
