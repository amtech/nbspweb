...
<script>
  ...
  var jqgrid=jQuery("#workordergrid").jqGrid({    
		url: "${ctx}/analysis/workOrderAnalysisAction!processtimelist.action",   
		datatype: "json",    
		mtype: 'GET',
		rownumbers: true,
		colNames:['ID','名称','超时工单总历时（小时）','超时工单平均历时（小时）','及时处理工单平均历时（小时）','工单总历时（小时）','工单平均历时（小时）'],
		colModel:[
		          ...
		          {name:'INTIME_AVG_LEN',id:'INTIME_AVG_LEN',sortable:false},
...
<body>
	<form id="troubleForm" name="troubleForm">
		<div id="header">
			<div class="title_bg">
				<div id="title" class="title">当前位置-工单历时统计</div>
			</div>
